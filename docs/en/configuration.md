...
**adapters:** Declares which implementation each adapter type should use. The Core gives the active adapter for each type only to the infrastructure plugin responsible for that type, not to every plugin. For example, the `storage` adapter is given only to the storage infrastructure plugin, and that plugin builds StorageService on top of it.
...
**Only the Core reads it.** This file is read only by the Core. No plugin can access it directly. The Core extracts the final configuration for each plugin and the UI from this file and gives it to that plugin or the UI at startup.
