...
3. Initialization ← Injecting configuration and registering in the registry; upon successful registration, the `core:part-loaded` message is published on the system bus
4. Stopping ← Notifying, waiting for operations to complete, removing from the registry; after removal, the `core:part-unloaded` message is published on the system bus
...
In this architecture, buses transfer messages. At the bus level, the architecture does not technically distinguish between an event, request, command, or response. All of these are messages, and their meaning is defined by the message `type` and the contracts of the parts.

### Bus Types

```text
System Bus  ← General messages that all authorized parts can hear
Private Bus ← Private messages of each part declared in the manifest
```
...
Every message must have a `type`. Other fields are added depending on the message's needs. The Core or the bus completes the technical fields when the message is published.
...
* `source` is the name of the publishing part and is set by the bus or the Core.
...
* Sensitive data such as passwords or tokens must not be published on the system bus.
...
Managing timeouts, missing responses, and retries is the responsibility of the part that published the original message. The bus may provide helper utilities for this, but those helpers must still use normal messages internally.
