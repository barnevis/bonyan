...
**adapters:** مشخص می‌کند هر نوع آداپتور کدام پیاده‌سازی را استفاده کند. هسته آداپتور فعال هر نوع را فقط در اختیار افزونه زیرساختی مربوط به همان نوع قرار می‌دهد، نه همه افزونه‌ها. برای مثال آداپتور `storage` فقط به افزونه زیرساختی ذخیره‌سازی داده می‌شود و آن افزونه است که StorageService را بر اساس آن می‌سازد.
